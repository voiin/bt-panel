...
if [[ "$(echo "$sys" | grep "$cen")" != "" ]];then
	wget -N --no-check-certificate "https://raw.githubusercontent.com/voiin/bt-panel/master/cen.sh" && bash cen.sh
else
	if [[ "$(echo $sys | grep "$un")" != "" ]] || [[ "$(echo $sys | grep "$de")" != "" ]];then
		wget -N --no-check-certificate "https://raw.githubusercontent.com/voiin/bt-panel/master/den.sh" && bash den.sh
	else
		echo "该脚本不支持此系统！"
		exit
	fi
